...
const Home = () => {
  ...
  const handleFilter = (selectedValue)=>{
    ...
    if(getList !== 'undefined'){
      ...
      let filteredArray = [];
      getList.forEach((item)=>{
        if(item.tags.includes(selectedValue))
  ...
  return (
    <section>
        {
          showFilter && <div className='filter-alert' onClick={handleResetFilter}> Filter has been applied, click to reset </div>
        }
        <div className='tags-box'>
          ...
          <div className='tag-container'>
            {
              tags?.map((item)=>{
                return <p className='post-tag' onClick={()=>handleFilter(item)} >{item}</p>
              })
            }
          </div>
        </div>
